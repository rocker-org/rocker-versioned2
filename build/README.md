...
## Publishing core images

The production publish orchestration lives in `.github/workflows/core.yml`. It reads
the generated version matrix and Bake files, builds each target/platform on its native
GitHub-hosted runner, and publishes Docker Hub and GHCR manifests only after all of a
target's platform builds succeed. Bake files remain the source of truth for target
order, platforms, and tags.

The Makefile's `bake-json-*` entry points are retained for local builds and static
validation. They do not implement the production digest collection, manifest
finalization, or registry verification flow.
